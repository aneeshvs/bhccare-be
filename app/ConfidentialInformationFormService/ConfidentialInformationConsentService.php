...
class ConfidentialInformationConsentService
{
    public function save(array $data): ConfidentialInformationConsent
    {
        // Step 1: Find or create the consent record
        $record = ConfidentialInformationConsent::firstOrNew([
            'confidential_information_form_id' => $data['confidential_information_form_id'],
        ]);

        $record->fill($data);

        // Step 2: Load related ConfidentialInformationForm (for user_id, client_type, etc.)
        ...
        if ($record->isDirty()) {
            ...
            Log::info('Consent Changes', [
                'dirty' => $changes,
                'original' => $original,
            ]);

            activity()
                ->useLog('confidential_information_consent')
                ->performedOn($record)
                ->causedBy(Auth::user())
                ->withProperties([
                    'attributes' => $changes,
                ...
                ->log('Confidential Information Consent record updated');
        }
